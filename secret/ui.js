...
class RecordList extends Div {
	constructor(parent,vault,onclick,ondblclick) {
		let list = super(parent,{className: 'records'})

		for (let record of vault.ids) {
			vault.retrieveRecord(record).then(rec => new RecordHTML(list,rec,onclick,ondblclick))
		}
		return list
	}
}
